    ...
    <div className="w-full max-w-xl mx-auto relative" ref={containerRef}>
      <div className="relative">
        <Input
          ...
          className="pl-3 pr-9 py-1.5 h-10 md:h-11 text-base rounded-md focus-visible:ring-offset-0 w-full"
        />
        <div className="absolute right-1 top-1/2 -translate-y-1/2 h-9 w-9">
          <AnimatePresence mode="popLayout">
            {query?.length > 0 ? (
              <motion.div
                ...
                className="cursor-pointer rounded-md hover:bg-accent w-full h-full flex items-center justify-center"
              ...
              <motion.div
                key="search"
                className="rounded-md hover:bg-accent w-full h-full flex items-center justify-center"
                initial={{ y: -20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                exit={{ y: 20, opacity: 0 }}
                transition={{ duration: 0.2 }}>
